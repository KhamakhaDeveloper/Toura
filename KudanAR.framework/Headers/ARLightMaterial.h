...
/**
 ...
 lightMaterial.colour.value = [ARVector3 vectorWithValuesX:1.0 Y:1.0 Z:1.0];
 lightMaterial.diffuse.texture = [[ARTexture alloc] initWithUIImage: [UIImage imageNamed:@"diffuseTexture.jpg"]];
 lightMaterial.ambient.value = [ARVector3 vectorWithValuesX:0.2 Y:0.2 Z:0.2];
 lightMaterial.normal.texture = [[ARTexture alloc] initWithUIImage: [UIImage imageNamed:@"normalTexture.jpg"]];
 @endcode
 */
...
/**
 ...
 Can be set as a constant value or as a texture.
 When set as a constant value using an ARVector3, this acts as an emission factor, which causes the material to effectively light itself with the given colour of light.
 When set using an ARTexture, it behaves as an Occlusion Map, where white areas of the map are areas of the texture are those that should have ambient light and dark areas of the map are areas of the texture that shouldn't have ambient light.
 Setting it with a texture is useful for objects with creases or folds, as light wouldn't naturally reach the entire object. A hood is a good example of such an object.
 */
